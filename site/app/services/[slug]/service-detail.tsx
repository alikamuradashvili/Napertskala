...
const services: Record<ServiceSlug, Service> = {
  ...
  industrial: {
    icon: Building2,
    ka: {
      label: 'სამრეწველო მომსახურება',
      title: 'დიდი აგრეგატები, გენერატორები და დანადგარები',
      lead: 'ტექნიკური მომსახურება იმ დანადგარებისთვის, რომელთა საიმედო მუშაობაც თქვენი ობიექტისთვის მნიშვნელოვანია.',
      summary: 'ვაფასებთ მექანიკურ და ელექტრო ნაწილებს, ვპოულობთ გაუმართაობას და ვგეგმავთ შეკეთებას ხელმისაწვდომი ნაწილებისა და სამუშაო პირობების მიხედვით.',
      features: [['გენერატორების მომსახურება', 'დიდი გენერატორების შემოწმება, დიაგნოსტიკა და ტექნიკური მომსახურება.'], ['სამრეწველო დანადგარები', 'საწარმოო მოწყობილობების მექანიკური და ელექტრო პრობლემების შეფასება.'], ['აგრეგატების შეკეთება', 'დაზიანებული კვანძების, სამაგრებისა და დამხმარე კონსტრუქციების აღდგენა.'], ['პროფილაქტიკური შემოწმება', 'ხილული ცვეთის, შეერთებებისა და სამუშაო მდგომარეობის შეფასება.'], ['ლითონის დეტალების დამზადება', 'საჭირო ჩარჩოების, სამაგრებისა და დამხმარე დეტალების შედუღებით დამზადება.'], ['ობიექტზე მომსახურება', 'გასვლა საწარმოსა და სამშენებლო ობიექტზე შეთანხმებით.']],
      process: [['მოწყობილობის მონაცემები', 'მოგვაწოდეთ მოდელი, ფოტოები და პრობლემის მოკლე აღწერა.'], ['შემოწმება', 'ვადგენთ რომელი კვანძი ან სისტემა საჭიროებს მუშაობას.'], ['სამუშაო გეგმა', 'ვათანხმებთ შეკეთების გზას, მასალებსა და ვადას.'], ['მომსახურება', 'ვასრულებთ სამუშაოს და ვამოწმებთ დანადგარის მდგომარეობას.']],
    },
    en: {
      label: 'Industrial service',
      title: 'Large generators, units and industrial equipment',
      lead: 'Technical service for the equipment your site depends on.',
      summary: 'We assess mechanical and electrical components, identify faults and plan repairs around available parts and operating conditions.',
      features: [['Generator service', 'Inspection, diagnostics and maintenance of large generators.'], ['Industrial equipment', 'Assessment of mechanical and electrical issues in production equipment.'], ['Unit repair', 'Restoration of damaged assemblies, mounts and support structures.'], ['Preventive inspection', 'Assessment of visible wear, connections and operating condition.'], ['Custom metal components', 'Welded fabrication of required frames, mounts and supporting details.'], ['On-site service', 'Visits to factories and construction sites by agreement.']],
      process: [['Equipment details', 'Send the model, photos and a short description of the problem.'], ['Inspection', 'We determine which component or system needs attention.'], ['Work plan', 'We agree on the repair method, materials and timing.'], ['Service', 'We complete the work and check the equipment condition.']],
    },
  },
  diagnostics: {
    icon: Car,
...
export default function ServiceDetail({ slug }: { slug: ServiceSlug }) {
  ...
  const phone = cms?.settings?.phone || '+995574814088';
  ...
  const visibleGallery = [...gallery, ...(cms?.gallery ?? []).map((item) => ({ src:item.url, ka:item.alt_ka || item.filename, en:item.alt_en || item.filename, tall:false }))];
  const scrollGallery = (direction: -1 | 1) => setGalleryIndex((current)=>(current+direction+visibleGallery.length)%visibleGallery.length);
  useEffect(() => { if(slug!=='welding'||visibleGallery.length<2)return;const timer=window.setInterval(()=>setGalleryIndex((current)=>(current+1)%visibleGallery.length),3600);return()=>window.clearInterval(timer); }, [slug,visibleGallery.length]);
