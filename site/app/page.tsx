...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#090a0b] text-white">
      ...
      <section id="services" className="bg-[#111214] py-16 sm:py-32"><div className="mx-auto max-w-7xl px-4 sm:px-5 lg:px-8"><div className="mb-8 max-w-3xl sm:mb-12"><h2 className="text-3xl font-black sm:text-6xl">{t.servicesTitle}</h2><p className="mt-4 text-base leading-7 text-zinc-400 sm:mt-5 sm:text-lg">{t.servicesLead}</p></div><div className="grid gap-4 md:grid-cols-2">{t.services.map(([title,desc,tag],i) => ({ title, desc, tag, Icon: icons[i], slug: serviceSlugs[i], index: i })).filter((service) => serviceEnabled(service.slug)).map(({ title, desc, tag, Icon, slug }) => { const servicePhotos = servicePhotoGroups[slug] ?? []; const photoCount = servicePhotos.length; const servicePhoto = photoCount ? servicePhotos[servicePhotoIndex % photoCount] : null; return <a key={title} href={`/services/${slug}`} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-[#0b0c0e] p-5 transition hover:-translate-y-1 hover:border-orange-500/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 sm:rounded-3xl sm:p-9">{servicePhoto && <div className="relative -mx-5 -mt-5 mb-5 aspect-[1.8/1] overflow-hidden border-b border-white/10 sm:-mx-9 sm:-mt-9 sm:mb-7 sm:aspect-[2.15/1]"><Image key={servicePhoto} src={servicePhoto} alt={lang === 'ka' ? `ნაპერწკალას ${title} სერვისის ფოტო` : `Napertskala ${tag} service photo`} fill sizes="(max-width: 768px) 100vw, 50vw" className="service-card-photo object-cover opacity-80 transition duration-500 group-hover:scale-105 group-hover:opacity-100" unoptimized={servicePhoto.startsWith('/api/')} /><div className="absolute inset-0 bg-gradient-to-t from-[#0b0c0e] via-transparent to-transparent"/><span className="absolute right-3 top-3 rounded-full bg-black/75 px-3 py-1 text-[10px] font-black text-orange-300 sm:right-4 sm:top-4">{String((servicePhotoIndex % photoCount)+1).padStart(2,'0')} / {String(photoCount).padStart(2,'0')}</span></div>}<div className="mb-7 flex items-center justify-between sm:mb-10"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-orange-500 text-black sm:h-14 sm:w-14"><Icon className="h-6 w-6 sm:h-7 sm:w-7"/></span><span className="text-[10px] font-bold uppercase tracking-[.22em] text-zinc-600">{tag}</span></div><h3 className="text-xl font-black sm:text-2xl">{title}</h3><p className="mt-3 max-w-md pr-7 text-sm leading-6 text-zinc-400 sm:pr-8 sm:text-base sm:leading-7">{desc}</p><ArrowRight className="absolute bottom-6 right-6 h-5 w-5 text-orange-500 transition group-hover:translate-x-1 sm:bottom-8 sm:right-8"/></a>})}</div></div></section>
