...
export async function POST(request: Request) {
  ...
  const gallery=String(form.get('gallery')??'welding');if(!isServiceSlug(gallery))return json({error:'Choose a valid service.'},400);
  ...
  if(String(form.get('addToGallery')??'')==='true'){const last=await db.prepare('SELECT COALESCE(MAX(sort_order),-1) AS position FROM gallery_items WHERE gallery=?').bind(gallery).first<{position:number}>();await db.prepare('INSERT INTO gallery_items(media_id,gallery,sort_order,created_at) VALUES(?,?,?,?)').bind(id,gallery,Number(last?.position??-1)+1,now).run();}
  await audit(auth.user.id,'upload_media','media',id,{filename:file.name,size:file.size,gallery,addToGallery:String(form.get('addToGallery')??'')==='true'});return json({ok:true,id,url:`/api/media/${id}`,gallery},201);
}
